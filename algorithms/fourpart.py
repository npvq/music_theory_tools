...
class SATB(object):
	...
	def _generate_voicings(self, chordMembers):
		...
		for sn, an, tn in permutations(chordMembers, 3):
			for s in self._generate_pitches(Pitch(sn), lb=self.ranges[3][2], ub=self.ranges[3][3]):
				...
				for b in self._generate_pitches(Pitch(bass), lb=max(self.ranges[0][2], t.transpose('-P15')), ub=min(self.ranges[0][3], t.transpose('-m2'))):
	...
	def voiceLeadingCost(chord1, rm1, chord2, rm2):
		...
		if LT in chord1.pitchNames:
			...
			if chord2[lt_idx] != chord1[lt_idx] and chord2[lt_idx] != Pitch(ctx_scale[Solfege.DO], octave=above(ctx_scale[Solfege.DO], chord1[lt_idx])): # ti->ti or ti->do have no cost
	...
	def voicePhraseDP(self, phrase): # phrase: one phrase of roman numerals (presumably with some sort of cadence)
		...
		bar = self.confidence * ( min(DP[0])[0] + self.first_chord_buffer//(len(V[0])*len(V[1])) )
		for j in range(len(V[0])):
			if DP[0][j][0] > bar:
				Mask[0][j] = False
		...
		for i in range(1, L):
			...
			# Mask updating (pruning)
			bar = self.confidence * (min(DP[i])[0] + self.buffer) 
			for j in range(len(V[i])):
				if DP[i][j][0] > bar:
					Mask[i][j] = False
